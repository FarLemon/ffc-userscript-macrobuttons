// ==UserScript==
// @name         FFC-[REDACTED]Buttons
// @version      1.0
// @description  An updated version of [REDACTED]'s [REDACTED]ButtonBar, with some added features
// @author       messegjo
// @match        *://*/*
// @grant        GM_setValue
// @grant        GM_getValue
...
// ========================= Global ========================= //
const STORAGE_KEY = 'ffc_dsb_command_bar';
...
GM_addStyle(`
    ...
    .ffc-dsb-wrapper * { box-sizing: border-box; }
    .ffc-dsb-bar {
        width: fit-content;
        display: flex;
        gap: 1em;
    ...
    .ffc-dsb-button, .ffc-dsb-groupButton {
        height: 2em;
        padding-inline: 1em;
        background: #1e1e2e;
        color: #f5e0dc;
    ...
    .ffc-dsb-buttonColored {
    }
    .ffc-dsb-groupButton {
        border-radius: 0.25em;
    }
    .ffc-dsb-groupButton-active {
        background: #45475a;
    }
`);
...
wrapper.className = 'ffc-dsb-wrapper';
document.body.appendChild(wrapper);

const mainBar = document.createElement('div');
mainBar.className = 'ffc-dsb-bar ffc-dsb-mainBar';
wrapper.appendChild(mainBar);

const controlsGrp = document.createElement('div');
controlsGrp.className = 'ffc-dsb-bar ffc-dsb-controls';
mainBar.appendChild(controlsGrp);

const buttonsGrp = document.createElement('div');
buttonsGrp.className = 'ffc-dsb-bar ffc-dsb-buttons';
...
GM_addStyle(`
    ...
    .ffc-dsb-modal input,
    .ffc-dsb-modal select,
    .ffc-dsb-modal textarea {
        width: 100%;
        box-sizing: border-box;
        padding: 4px;
        font-size: 12px;
    }
    .ffc-dsb-modal-actions {
        display: flex;
    ...
    .ffc-dsb-settingsModal-content h3 {
        margin-top: 0;
        font-size: 1.2rem;
    }
`);
...
settingsBtn.className = 'ffc-dsb-button ffc-dsb-buttonColored';
...
editBtn.className = 'ffc-dsb-button ffc-dsb-buttonColored';
editBtn.innerHTML = '✎';
...
editBtn.addEventListener('mousedown', (e) => {
    ...
    if (isEditMode) {
        editBtn.classList.add('ffc-dsb-groupButton-active');
    } else {
        editBtn.classList.remove('ffc-dsb-groupButton-active');
    }
    renderBars();
    enableDrag();
});
...
function openSettingsMenu() {
    ...
    modal.classList.add('ffc-dsb-settingsModal');

    modal.innerHTML = `
        <div class="ffc-dsb-settingsModal-content" style="max-width: 500px;">
            <h3>⚙️ Settings</h3>
            <div style="display: flex; flex-direction: column; gap: 1em;">
                <button id="ffc-export" class="ffc-dsb-button">Export Buttons</button>
                <input type="file" id="ffc-import-file" accept=".json" style="display:none;">
                <button id="ffc-import" class="ffc-dsb-button">Import Buttons (from file)</button>
                <textarea id="ffc-import-text" placeholder="Or paste JSON here" style="width:100%; height:6em;"></textarea>
                <div style="display: flex; gap: 1em; justify-content: flex-end;">
                    <button id="ffc-import-apply" class="ffc-dsb-button">Apply Import</button>
                    <button id="ffc-close-settings" class="ffc-dsb-button">Close</button>
                </div>
            </div>
        </div>
    `;
    ...
    exportBtn.onclick = async () => {
        ...
        a.href = url;
        a.download = 'FFC_[REDACTED]Buttons_Data.json';
        a.click();
...
function openEditModal({ item = null, parentGroupId = null }) {
    ...
    overlay.classList.add('ffc-dsb-modal-overlay');

    const modal = document.createElement('div');
    modal.classList.add('ffc-dsb-modal');
    modal.innerHTML = `
        ...
        <div class="ffc-dsb-modal-actions">
            <button id="ffc-delete" class="ffc-dsb-button ffc-dsb-buttonColored">Delete</button>
            <button id="ffc-save" class="ffc-dsb-button ffc-dsb-buttonColored">Save</button>
            <button id="ffc-cancel" class="ffc-dsb-button">Cancel</button>
        </div>
    `;
...
function renderRow(buttons, rowIndex, container, parentGroupId = null) {
    ...
    if (isEditMode) {
        ...
        addBtn.className = 'ffc-dsb-button ffc-dsb-buttonColored add-button';
        addBtn.textContent = '+';
    ...
    for (const item of buttons) {
        ...
        if (item.type === 'button') {
            btn.classList.add('ffc-dsb-button');
            ...
            btn.classList.add('ffc-dsb-groupButton');
            if (activeGroupsPerRow[rowIndex] === item.id)
                btn.classList.add('ffc-dsb-groupButton-active');
            btn.addEventListener('mousedown', (e) => {
                e.preventDefault();
                e.stopPropagation();
...
function renderBars() {
    ...
    document.querySelectorAll('.ffc-dsb-groupBar').forEach(bar => bar.remove());
    ...
    for (let rowIndex = 0; rowIndex < activeGroupsPerRow.length; rowIndex++) {
        ...
        if (group) {
            ...
            groupBar.className = 'ffc-dsb-bar ffc-dsb-groupBar';
...
function setupSortables() {
    ...
    const bars = [buttonsGrp, ...document.querySelectorAll('.ffc-dsb-groupBar')];
